...
async fn disconnect_on_sigint<T, F>(peripheral: &btleplug::platform::Peripheral, future: F) -> Result<T>
where
...
fn print_command_output(format: OutputFormat, output: &dyn crate::sensor::CommandOutput) -> Result<()> {
    match format {
...
async fn history(cmd: C1PlusHistoryCmd) -> Result<()> {
    ...
    with_disconnect(&connected.peripheral, async {
        let history_result = disconnect_on_sigint(&connected.peripheral, async {
            ...
            write_request(peripheral, &connected.write_char, &meta_req, cmd.target.verbose).await?;
            ...
            while records.len() < total_count as usize {
                ...
                write_request(peripheral, &connected.write_char, &chunk_req, cmd.target.verbose).await?;
